...
-- Find House And Room Average Value Per Year, BedRooms And Acreage Group
select 
year(SalesDateConverted) as SaleYear, bedrooms,
case 
	when Acreage < 0.2 then '0 - 0.199'
	when Acreage < 0.3 then '0.2 - 0.299'
	when Acreage < 0.4 then '0.3 - 0.399'
	else '0.4+'
end as AcreageGroup,
		round(avg(Acreage),3) as AverageAcreage, format(avg(saleprice),'#,#') as AverageSales, 
...
having count(saleprice) > 10 and bedrooms >0
order by year(SalesDateConverted) , bedrooms 
