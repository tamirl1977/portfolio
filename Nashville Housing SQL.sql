...
-- Calculate the Percent Of Every Sale By City And number Of Rooms 

	-- Create A Temp Table To Hold The Average Sale Price Of All Sales By Cuty & Bedrooms
...
SalePrice int
	...
	-- Insert Average Data To Temp Table
...
group by year(SalesDateConverted), PropertySpllitCity, Bedrooms
	...
	-- Calculate Sale As A Percentage Of Average Sales By Bedrooms In The City

select a.[UniqueID ], a.PropertySpllitCity, year(a.SalesDateConverted) as SaleYear, a.Bedrooms , format(a.SalePrice,'#,#') as SalePrice,
	   format(b.SalePrice,'#,#') as AverageSalePrice, format(a.SalePrice / b.SalePrice, 'p') as Percenta
from dbo.Main_DB a
