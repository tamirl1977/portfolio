...
-- Rank Months By The Average Sales

select month(salesdateconverted) as Month, format(count(salesdateconverted),'#,#') as SalesCount , format(avg(saleprice),'#,#') as AverageSales,
	   DENSE_RANK() over (order by avg(saleprice) desc) as MonthAverageSaleRank
from dbo.Main_DB
group by month(salesdateconverted)
order by month(salesdateconverted)



-- Calculate the Percent Of Every Sale By Country, City And number Of Rooms Using A Temp Table

	-- Create A Temp Table To Hold The Average Sale Price Of All Sales By City, Year And Bedrooms
drop table if exists #Temp_City_Bedrooms_AverageSalePrice
create table #Temp_City_Bedrooms_AverageSalePrice(
SaleYear int,
PropertySpllitCity  varchar(100),
Bedrooms int,
SalePrice int
)

	-- Insert Average Data To City Average Temp Table
insert into #Temp_City_Bedrooms_AverageSalePrice
select year(SalesDateConverted), PropertySpllitCity, Bedrooms , avg(SalePrice)
from dbo.Main_DB
where Bedrooms is not null
group by year(SalesDateConverted), PropertySpllitCity, Bedrooms

	-- Create A Temp Table To Hold The Average Sale Price Of All Sales In Country By Year And Bedrooms
drop table if exists #Temp_Country_Bedrooms_AverageSalePrice
create table #Temp_Country_Bedrooms_AverageSalePrice(
SaleYear int,
Bedrooms int,
SalePrice int
)

	-- Insert Average Data To Country Average Temp Table
insert into #Temp_Country_Bedrooms_AverageSalePrice
select year(SalesDateConverted), Bedrooms , avg(SalePrice)
from dbo.Main_DB
where Bedrooms is not null
group by year(SalesDateConverted), Bedrooms

	-- Calculate Sale As A Percentage Of Average Sales By Bedrooms In The City & Country

select a.[UniqueID ], a.PropertySpllitCity, year(a.SalesDateConverted) as SaleYear, a.Bedrooms , format(a.SalePrice,'#,#') as SalePrice,
	   format(b.SalePrice,'#,#') as AverageSalePriceInCityAndBedrooms, format(a.SalePrice / b.SalePrice, 'p') as PercentCity,
	   format(c.SalePrice,'#,#') as AverageSalePriceInCountryAndBedrooms, format(a.SalePrice / c.SalePrice, 'p') as PercentCountry
from dbo.Main_DB a
join #Temp_City_Bedrooms_AverageSalePrice b
on a.PropertySpllitCity=b.PropertySpllitCity and b.SaleYear = year(a.SalesDateConverted) and a.Bedrooms = b.Bedrooms
join #Temp_Country_Bedrooms_AverageSalePrice c
on c.SaleYear = year(a.SalesDateConverted) and a.Bedrooms = c.Bedrooms
where a.Bedrooms is not null and a.Bedrooms = '3' and year(a.SalesDateConverted) = '2013'
order by 2,3,4
