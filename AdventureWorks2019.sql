...
-- Full Product, SubProduct & Category List
...
-- Find The Number Of Products In Each Category (To Check If The Following Pivot Is Correct)
...
-- Sort Stores By Percent Of TotalDue From All Sales In 10.2011 
...
select c.StoreID, s.Name, soh.ShipDateConverted , format(sum(soh.TotalDue),'#,#') as SumTotalDue, 
	...
	sum(cast(soh.TotalDue as float))/(select sum(cast(totaldue as float)) from AdventureWorks2019.Sales.SalesOrderHeader where year(ShipDateConverted) = '2011' and month(ShipDateConverted) = '10') * 100 as PercentOfALL
...
select c1.StoreID, s1.Name, soh1.ShipDateConverted , format(sum(soh1.TotalDue),'#,#') as SumTotalDue, 
	...
	sum(cast(soh1.TotalDue as float))  /(select sum(cast(soh2.totaldue as float))
		...
		full join AdventureWorks2019.Sales.Customer c2 on c2.CustomerID = soh2.CustomerID
		full join AdventureWorks2019.Sales.Store s2 on s2.BusinessEntityID = c2.StoreID
		where year(soh2.ShipDateConverted) = '2011' and month(soh2.ShipDateConverted) = '10' and s2.Name is not null) * 100 as PercentOfALL
...
-- Get Active Employees (SalariedFlag=1) Current (MAX) Rate And Compare It To Job Average Rate In The Job
